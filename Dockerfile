...
RUN apk update --no-cache \
    ...
    && sed -i "s|font-weight: bold;|font-weight:bold;font-family: LiberationSans-Bold;|g" "$FILE" sed -i "s|font-style: italic;|font-style: italic;font-family: LiberationSans-Italic;|g" "$FILE" \
    && sed -i "/^DEFAULT_CSS/cfrom os import path, listdir\ndejavu = '/usr/share/fonts/ttf-liberation'\nfonts = {file.split('.')[0]: path.join(dejavu, file) for file in listdir(dejavu) if file.endswith('.ttf')}\nDEFAULT_CSS = '\\\n'.join(('@font-face { font-family: \"%s\" \
    && src: \"%s\";%s%s }' % (name, file, ' font-weight: \"bold\";' if 'bold' in name.lower() else '', ' font-style: \"italic\";' if 'italic' in name.lower() or 'oblique' in name.lower() else '') for name, file in fonts.items())) + \"\"\"" "$FILE"; done
